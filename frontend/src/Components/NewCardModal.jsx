...
export default function NewCardModal({ projectId, onCreated, onClose }) {
  const [type, setType] = useState('FEATURE'); // FEATURE | BUG | IDEA | SKETCH
      ...
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="form-grid">
          ...
          <label>
            ...
            <textarea rows="4" value={desc} onChange={e => setDesc(e.target.value)} placeholder="Details..." />
          </label>
        ...
        <div style={{display:'flex', gap:'.5rem', justifyContent:'flex-end', marginTop:'.5rem'}}>
          ...
          <button className="btn btn-primary" onClick={save} disabled={!title.trim()}>Create</button>
        </div>
      </div>
    </div>
  );
